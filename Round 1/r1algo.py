...
# Needed for visualizer and backtester provided by Jmerle
class Logger:
...
class MarketMaking(Strategy): 
    def __init__(self, symbol: Symbol, limit: int) -> None:
        ...
        self.prices = deque()
        self.window_size = 10
        self.trend_window = 20  # short-term trend memory
    ...
    def act(self, state: TradingState) -> None:
        ...
        max_buy_price = true_value - 2 if position > self.limit * 0.5 else true_value
        min_sell_price = true_value + 2 if position < self.limit * -0.5 else true_value
...
#True price hovers around 10,000
class RainforestResinStrategy(MarketMaking):
    def get_true_value(self, state: TradingState) -> int:
        return 10_000

#Market make around populate price of Kelp
class KelpStrategy(MarketMaking):
    ...
    def get_true_value(self, state: TradingState) -> int:
        ...
        popular_buy_price = max(buy_orders, key=lambda tup: tup[1])[0]
        popular_sell_price = min(sell_orders, key=lambda tup: tup[1])[0]
        market_estimate = (popular_buy_price + popular_sell_price) / 2
        
        # attempt to use trends but didn't add anything 
        # self.prices.append(market_estimate)
...
class Trader:
    def __init__(self) -> None:
        ...
        self.strategies = {symbol: clazz (symbol, limit[symbol]) for symbol, clazz in {
